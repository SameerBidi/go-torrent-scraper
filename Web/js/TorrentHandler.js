...
function searchTorrents(site_id) {
	...
	sendGet('getTorrents', {"siteId": site_id, "searchKey": search_key, "safeSearch": safe_search}, 
		function (response) {
			showProgress(false);
			...
			response.forEach(function(torrent, index) {
				...
				tDataCollapse.on("show.bs.collapse", function() {
					...
					sendGet('getTorrentData',  {"siteId": site_id, "link": torrent.link}, 
						function (response) {
		...
		(
			{
				...
				url: 'http://samcloud.tplinkdns.com:50001/' + to,
				data: data,
				dataType: 'json',
				cache: false,
				timeout: 60000,
